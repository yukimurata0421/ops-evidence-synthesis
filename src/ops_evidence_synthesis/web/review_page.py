...
from ops_evidence_synthesis.pipeline_progress import analysis_pipeline_status_from_store
from ops_evidence_synthesis.synthesis.multi_ai import (
    SCORE_NOTE,
    finding_impact_from_synthesis,
    model_run_artifacts_from_records,
    synthesize_multi_ai,
)
...
def configure_review_page_store(store_factory: Callable[[], Any]) -> None:
...
def _store() -> Any:
    if _STORE_FACTORY is None:
        raise RuntimeError("review page store factory is not configured")
...
def _review_targets_page(
    ...
    *,
    ...
    if not canonical_target_set.get("targets") and legacy_targets:
        canonical_target_set = {
            "schema_version": "legacy_review_target_set.v1",
            "summary": legacy_summary,
            "targets": legacy_targets,
            "canonical_review_graph": canonical_graph if isinstance(canonical_graph, dict) else {},
        }
    summary = dict(canonical_target_set.get("summary") or legacy_summary)
    targets = list(canonical_target_set.get("targets") or [])
    cards = _canonical_review_graph_cards(canonical_graph) or _review_graph_cards(targets) or (
...
def _canonical_review_graph_for_ui(
    ...
    try:
        ...
        if snapshot_response and not model_artifacts:
...
def _review_summary_for_ui(evidence_sha256: str) -> dict[str, Any]:
    ...
    target_set = {}
    if not graph and hasattr(store, "list_review_targets"):
        target_set = store.list_review_targets(limit=20, evidence_sha256=evidence_sha256, pending_only=False)
    target_summary = target_set.get("summary") if isinstance(target_set.get("summary"), dict) else {}
    target_rows = [row for row in target_set.get("targets") or [] if isinstance(row, dict)]
    ...
    return {
        ...
        "status": "ok" if graph or target_rows else "not_found",
        "message": "" if graph or target_rows else "No persisted canonical review graph is available yet.",
        "finding": {
            "title": str(finding.get("title") or display_summary.get("title") or first_target.get("title") or ""),
            "impact": str(
                finding.get("impact")
                or display_summary.get("impact")
                or first_target.get("impact_summary")
                or first_target.get("core_claim")
                or ""
            ),
        },
        "review": {
            "primary_targets": int(summary.get("primary_count") or target_summary.get("primary_review_targets") or 0),
            "validation_targets": int(summary.get("validation_count") or target_summary.get("validation_targets") or 0),
            "monitor_only": int(summary.get("monitor_only_count") or target_summary.get("monitor_only") or 0),
            "auto_archived": int(summary.get("auto_archived_count") or target_summary.get("auto_archived") or 0),
        },
        "providers": {
        ...
        "canonical_graph_status": str(snapshot_response.get("canonical_graph_status") or ("legacy_targets" if target_rows else "not_found")),
...
def _model_run_artifacts_for_ui(evidence_sha256: str | None) -> list[dict[str, Any]]:
    ...
    return model_run_artifacts_from_records(runs, parsed)
...
render_review_targets_page = _review_targets_page
review_summary_for_ui = _review_summary_for_ui
latest_canonical_graph_response = _latest_canonical_graph_response
model_run_artifacts_for_ui = _model_run_artifacts_for_ui
multi_ai_synthesis_for_ui = _multi_ai_synthesis_for_ui
canonical_review_graph_cards = _canonical_review_graph_cards
target_set_from_canonical_graph = _target_set_from_canonical_graph
bundle_lineage_summary = _bundle_lineage_summary
evidence_request_planner_panel = _evidence_request_planner_panel
pipeline_progress_panel = _pipeline_progress_panel
html_escape = _html

__all__ = [
    "bundle_lineage_summary",
    "canonical_review_graph_cards",
    "configure_review_page_store",
    "evidence_request_planner_panel",
    "html_escape",
    "latest_canonical_graph_response",
    "model_run_artifacts_for_ui",
    "multi_ai_synthesis_for_ui",
    "pipeline_progress_panel",
    "render_review_targets_page",
    "review_summary_for_ui",
    "target_set_from_canonical_graph",
]
