...
> 45,000行を全件サニタイズし、1,035のEvidence Itemを5モデルで解析しました。低い送信速度による停滞を主張したのは4モデルで、Gemmaはsilentです。healthyとstream activeという反証と、視聴者影響の証拠不足から、Validation Targetに止めます。

スコアを示しながら：

> スコアは原因確率ではなく、確認優先度です。
...
対象システム：`amazon-notify`。ここで`stream_v3`へは戻りません。
...
> amazon-notifyの最初の判断では、ユーザー影響の証拠が不足しています。OESはこの仮説を原因へ昇格させず、Human Gateで止め、次に必要な追加証拠を要求します。child Evidence Bundleを加えると判断状態が進み、Validation TargetをPrimary Candidateへ再評価します。
