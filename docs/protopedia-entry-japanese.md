...
raw log はローカルに残し、サニタイズ済みのEvidence BundleだけをSHA256で固定します。今回の公開主導線では、45,000行のstream_v3 runtimeログを1,012個のEvidence Itemにまとめ、Gemini、GPT OSS、Mistral、Qwen、Gemma 4の5つの実APIでチャンク解析を行い、0件のprimary candidateと11件のvalidation targetとして可視化します。amazon-notifyの44,944行レビューは、5/5 provider支持で1件のprimary candidateが出ても自動承認しない抑制例として残しています。
...
メインデモでは、45,000行のサニタイズ済みstream_v3 runtimeログを、coverage ledgerで全行追跡した上で1,012個のEvidence Itemへまとめ、Gemini、GPT OSS、Mistral、Qwen、Gemma 4の5つの実APIへprovider別チャンクで投げています。providerの支持は「真実」ではなく review work として扱い、stream_v3 runtimeでは 0 primary candidate / 11 validation target として残します。primary candidate がある別レビューでも、それは原因確定ではなく、人間が優先レビューすべき候補です。
