...
Gemini Enterprise Agent Platform を最初に走る reference provider とし、gpt-oss、Mistral、Qwen、GLMを adversarial cross-check として扱います。Geminiを真偽の正解ラベルにはしません。複数providerが同じ証拠を見ても、同意は「真実」ではなく review signal として扱います。沈黙や不一致は validation target として残します。
