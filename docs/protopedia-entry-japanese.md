...
raw log はローカルに残し、サニタイズ済みのEvidence BundleだけをSHA256で固定します。今回の公開主導線では、44,944行のサニタイズ済みDBコーパスを8,519個のEvidence Itemにまとめ、Gemini、GPT OSS、Mistral、Qwen、GLMの5つの実APIでチャンク解析を行い、Review Targetとして可視化します。
...
メインデモでは、44,944行のサニタイズ済みamazon-notifyログを、coverage ledgerで全行追跡した上で8,519個のEvidence Itemへまとめ、Gemini、GPT OSS、Mistral、Qwen、GLMの5つの実APIへprovider別チャンクで投げています。providerの支持は「真実」ではなく review work として扱い、原因・影響・運用結果が足りないものは validation target として残します。
