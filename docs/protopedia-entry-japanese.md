...
raw log はローカルに残し、サニタイズ済みのEvidence BundleだけをSHA256で固定します。今回の公開主導線では、45,000行のstream_v3 runtimeログを1,012個のEvidence Itemにまとめ、Gemini、GPT OSS、Mistral、Qwen、GLMの5つの実APIでチャンク解析を行い、3つのhuman-gated primary candidateを含むReview Targetとして可視化します。amazon-notifyの44,944行レビューは、5/5 provider支持でも自動昇格しない抑制例とMore Data Rescore Demoとして残しています。
...
- Gemini、GPT OSS、Mistral、Qwen、GLMでサニタイズ済みEvidence Itemをチャンク解析する
...
メインデモでは、45,000行のサニタイズ済みstream_v3 runtimeログを、coverage ledgerで全行追跡した上で1,012個のEvidence Itemへまとめ、Gemini、GPT OSS、Mistral、Qwen、GLMの5つの実APIへprovider別チャンクで投げています。providerの支持は「真実」ではなく review work として扱い、原因・影響・運用結果が足りないものは human-gated primary candidate / validation target として残します。
