...
- Public projection of that run is derived from the immutable recorded Provider outputs with the current stance-aware synthesis contract. Agreement uses distinct supporting Providers; participation, caveat, counter, and missing-evidence signals do not inflate the numerator. Re-derivation does not call a Provider API, and the historical pre-v2 public payload remains retained as an audit artifact.
